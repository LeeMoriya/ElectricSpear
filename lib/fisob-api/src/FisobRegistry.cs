        ...
        private AbstractPhysicalObject SaveState_AbstractPhysicalObjectFromString(On.SaveState.orig_AbstractPhysicalObjectFromString orig, World world, string objString)
        {
            ...
            if (fisobsByID.TryGetValue(array[1], out Fisob o) && array.Length > 2) {
                ...
                if (int.TryParse(coordParts[0], out int room) &&
                    ...
                    Debug.Log($"{nameof(ESFisobs)} : Corrupt world coordinate on object \"{id}\", type \"{o.ID}\"");
                    return null;
                }
                ...
                if (array.Length > 4) {
                    Debug.LogError($"{nameof(ESFisobs)} : Save data had more than 4 <oA> sections in \"{o.ID}\". Override `APO.ToString()` to return `this.SaveAsString(...)`.");
                    return null;
                }
                ...
                try {
                    ...
                    Debug.LogError($"{nameof(ESFisobs)} : {e}");
                    return null;
                }
            }
